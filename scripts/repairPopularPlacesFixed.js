// ===== SCRIPT DE RÉPARATION CORRIGÉ =====
// Fichier: scripts/repairPopularPlacesFixed.js
...
async function repairPopularPlaces() {
    try {
        console.log('🔧 === DÉBUT RÉPARATION PLACES POPULAIRES CORRIGÉE ===');
        ...
        for (const regionDetail of regionsDetails) {
            ...
            try {
                ...
                // ✅ CORRECTION : Analyser les fichiers avec logique corrigée
                ...
                files.forEach(fileName => {
                    ...
                    const pattern1Match = fileName.match(/place_(\d+)_(\d{13,})/);
                    if (pattern1Match) {
                        placeIndex = parseInt(pattern1Match[1]);
                        placeId = parseInt(pattern1Match[2]);
                        console.log(`✅ Pattern 1 détecté: index=${placeIndex}, id=${placeId}`);
                    } else {
                        // Pattern 2: place_1234567890_5678901234.webp (deux timestamps)
                        const pattern2Match = fileName.match(/place_(\d{13,})_(\d{10,})/);
                        if (pattern2Match) {
                            placeId = parseInt(pattern2Match[1]);
                            placeIndex = 0; // Défaut si pas d'index
                            console.log(`✅ Pattern 2 détecté: id=${placeId}, index défaut=${placeIndex}`);
                        } else {
                            // Pattern 3: place_0_1234567890_nom.webp (index + timestamp + nom)
                            const pattern3Match = fileName.match(/place_(\d+)_(\d{13,})_/);
                            if (pattern3Match) {
                                placeIndex = parseInt(pattern3Match[1]);
                                placeId = parseInt(pattern3Match[2]);
                                console.log(`✅ Pattern 3 détecté: index=${placeIndex}, id=${placeId}`);
                            } else {
                                console.log(`⚠️ Impossible d'extraire l'ID du fichier: ${fileName}`);
                            }
                        }
                    }
                ...
                // ✅ METTRE À JOUR LES PLACES POPULAIRES
                let placesUpdated = 0;
                ...
                for (let i = 0; i < regionDetail.popularPlaces.length; i++) {
                    ...
                    if (!bestMatch && place.id) {
                        for (const [index, images] of imageMap.entries()) {
                            const match = images.find(img => img.placeId.toString() === place.id.toString());
                            if (match) {
                                bestMatch = match;
                    ...
                    if (bestMatch) {
                        ...
                        const matchIndex = imageMap.get(bestMatch.placeIndex).indexOf(bestMatch);
                        if (matchIndex > -1) {
                            imageMap.get(bestMatch.placeIndex).splice(matchIndex, 1);
    ...
    try {
        ...
        for (const regionDetail of regionsDetails) {
            ...
            for (let i = 0; i < regionDetail.popularPlaces.length; i++) {
                ...
                // ✅ CORRECTION : Vérifier hasImage comme booléen
                if (place.hasImage === true) {
                    placesWithImages++;
                }
...
if (require.main === module) {
    console.log('🚀 Lancement du script de réparation corrigé...');
    repairPopularPlaces();
}
